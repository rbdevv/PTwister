...
public class twister extends Activity{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        CharacteristicPicker.setAdapter(bb);
              ...
              new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, Natures);
...
//		int SPIV = 0;
		...
		Integer PokedexNo = 0;
		Log.i("Ptwister", "About to get Pokedex # " + PokedexNoBox.getText().toString() );
		PokedexNo = Integer.parseInt(PokedexNoBox.getText().toString())-1;
		Log.i("Ptwister", "Pokedex # is" + PokedexNo);
		...
		for( int iv=1; iv<+31; iv++){
			if (Integer.parseInt((ATBox.getText().toString())) == Math.floor(((((iv + (2*BaseStats[1][PokedexNo]))*Level)/100)+5)*testnature)){
				ATIV.add(iv);
			}
		};
		for( int iv=1; iv<+31; iv++){
			if (Integer.parseInt((DFBox.getText().toString())) == Math.floor(((((iv + (2*BaseStats[2][PokedexNo]))*Level)/100)+5)*testnature)){
				DFIV.add(iv);
			}
		};
		for( int iv=1; iv<+31; iv++){
			if (Integer.parseInt((SABox.getText().toString())) == Math.floor(((((iv + (2*BaseStats[3][PokedexNo]))*Level)/100)+5)*testnature)){
				SAIV.add(iv);
			}
		};
		for( int iv=1; iv<+31; iv++){
			if (Integer.parseInt((SDBox.getText().toString())) == Math.floor(((((iv + (2*BaseStats[4][PokedexNo]))*Level)/100)+5)*testnature)){
				SDIV.add(iv);
			}
		};
		for( int iv=1; iv<+31; iv++){
			if (Integer.parseInt((SPBox.getText().toString())) == Math.floor(((((iv + (2*BaseStats[5][PokedexNo]))*Level)/100)+5)*testnature)){
				SPIV.add(iv);
			}
		};
